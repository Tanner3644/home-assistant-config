class UpcomingMediaCard extends HTMLElement {
  set hass(hass) {
    ...
    const line4_color =
      ...
      this.config.line_color ||
      defaultClr("var(--primary-text-color)", "#fff");
    const accent = this.config.accent_color || defaultClr("var(--primary-color)", "var(--third-background-color)");
    const border = this.config.border_color || defaultClr("#fff", "var(--third-background-color)");
    const shadows = conf =>
